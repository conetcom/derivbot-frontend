//#region node_modules/technicalindicators/lib/Utils/LinkedList.js
var Item = class {
...
var LinkedList = class {
	...
	unshiftCurrent() {
		...
		if (current === this._head || this._length < 2) return current && current.data;
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/FixedSizeLinkedList.js
/**
...
var FixedSizeLinkedList = class extends LinkedList {
	constructor(size, maintainHigh, maintainLow, maintainSum) {
		...
		this.periodSum = 0;
		if (!size || typeof size !== "number") throw "Size required and should be a number.";
	...
	add(data) {
		if (this.length === this.size) {
			...
			if (this.maintainSum) this.periodSum = this.periodSum - this.lastShift;
		...
		if (this.maintainSum) this.periodSum = this.periodSum + data;
	}
	*iterator() {
		this.resetCursor();
		while (this.next()) yield this.current;
	}
	calculatePeriodHigh() {
		this.resetCursor();
		if (this.next()) this.periodHigh = this.current;
		while (this.next()) if (this.periodHigh <= this.current) this.periodHigh = this.current;
	}
	calculatePeriodLow() {
		this.resetCursor();
		if (this.next()) this.periodLow = this.current;
		while (this.next()) if (this.periodLow >= this.current) this.periodLow = this.current;
	}
};
//#endregion
//#region node_modules/technicalindicators/lib/StockData.js
...
//#endregion
//#region node_modules/technicalindicators/lib/config.js
var config = {};
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/NumberFormatter.js
function format(v) {
	let precision = getConfig("precision");
	if (precision) return parseFloat(v.toPrecision(precision));
	return v;
}
//#endregion
//#region node_modules/technicalindicators/lib/indicator/indicator.js
...
//#endregion
//#region node_modules/technicalindicators/lib/moving_averages/SMA.js
var SMA = class extends Indicator {
	constructor(input) {
		...
		this.price.forEach((tick) => {
			...
			if (result.value !== void 0) this.result.push(this.format(result.value));
		});
	}
	nextValue(price) {
		var result = this.generator.next(price).value;
		if (result != void 0) return this.format(result);
	}
};
SMA.calculate = sma;
function sma(input) {
...
//#endregion
//#region node_modules/technicalindicators/lib/moving_averages/EMA.js
var EMA = class extends Indicator {
	constructor(input) {
		...
		var genFn = (function* () {
			var tick = yield;
			var prevEma;
			while (true) if (prevEma !== void 0 && tick !== void 0) {
...
//#endregion
//#region node_modules/technicalindicators/lib/moving_averages/WMA.js
var WMA = class extends Indicator {
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) if (data.length < period) data.push(yield);
			else {
				data.resetCursor();
				let result = 0;
				for (let i = 1; i <= period; i++) result = result + data.next() * i / denominator;
...
//#endregion
//#region node_modules/technicalindicators/lib/moving_averages/WEMA.js
	...
	constructor(input) {
		...
		var genFn = (function* () {
			...
			while (true) if (prevEma !== void 0 && tick !== void 0) {
				...
				tick = yield;
				prevEma = sma.nextValue(tick);
				if (prevEma !== void 0) tick = yield prevEma;
			}
		});
		this.generator = genFn();
		this.generator.next();
...
//#endregion
//#region node_modules/technicalindicators/lib/moving_averages/MACD.js
/**
	...
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				tick = yield {
					MACD: format(MACD),
					signal: signal ? format(signal) : void 0,
					histogram: isNaN(histogram) ? void 0 : format(histogram)
				};
				fast = fastMAProducer.nextValue(tick);
				slow = slowMAProducer.nextValue(tick);
			}
		...
		input.values.forEach((tick) => {
			var result = this.generator.next(tick);
			if (result.value != void 0) this.result.push(result.value);
		});
	}
	nextValue(price) {
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/AverageGain.js
	...
	constructor(input) {
		...
		this.generator = (function* (period) {
			...
			while (true) {
				...
				gain = gain > 0 ? gain : 0;
				if (gain > 0) gainSum = gainSum + gain;
				if (counter < period) counter++;
				else if (avgGain === void 0) avgGain = gainSum / period;
				else avgGain = (avgGain * (period - 1) + gain) / period;
				lastValue = currentValue;
				avgGain = avgGain !== void 0 ? format(avgGain) : void 0;
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/AverageLoss.js
	...
	constructor(input) {
		...
		this.generator = (function* (period) {
			...
			while (true) {
				...
				loss = loss > 0 ? loss : 0;
				if (loss > 0) lossSum = lossSum + loss;
				if (counter < period) counter++;
				else if (avgLoss === void 0) avgLoss = lossSum / period;
				else avgLoss = (avgLoss * (period - 1) + loss) / period;
				lastValue = currentValue;
				avgLoss = avgLoss !== void 0 ? format(avgLoss) : void 0;
...
//#endregion
//#region node_modules/technicalindicators/lib/oscillators/RSI.js
/**
	...
	constructor(input) {
		...
		this.generator = (function* (period) {
			...
			while (true) {
				...
				if (lastAvgGain !== void 0 && lastAvgLoss !== void 0) if (lastAvgLoss === 0) currentRSI = 100;
				else if (lastAvgGain === 0) currentRSI = 0;
				else {
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/SD.js
var SD = class extends Indicator {
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				if (mean) {
					let sum = 0;
					for (let x of currentSet.iterator()) sum = sum + Math.pow(x - mean, 2);
	...
	nextValue(price) {
		...
		if (nextResult.value != void 0) return this.format(nextResult.value);
	}
};
SD.calculate = sd;
function sd(input) {
...
//#endregion
//#region node_modules/technicalindicators/lib/volatility/BollingerBands.js
...
//#endregion
//#region node_modules/technicalindicators/lib/moving_averages/WilderSmoothing.js
		...
		var genFn = (function* (period) {
			...
			while (true) {
				if (counter < period) {
					...
					sum = sum + current;
					result = sum;
				} else result = result - result / period + current;
				current = yield result;
			}
		});
		this.generator = genFn(this.period);
...
//#endregion
//#region node_modules/technicalindicators/lib/directionalmovement/MinusDM.js
var MDM = class MDM extends Indicator {
	constructor(input) {
		super(input);
		var lows = input.low;
		var highs = input.high;
		var format = this.format;
		if (lows.length != highs.length) throw "Inputs(low,high) not of equal size";
		this.result = [];
		this.generator = (function* () {
			var minusDm;
			var current = yield;
...
//#endregion
//#region node_modules/technicalindicators/lib/directionalmovement/PlusDM.js
var PDM = class PDM extends Indicator {
	constructor(input) {
		super(input);
		var lows = input.low;
		var highs = input.high;
		...
		this.generator = (function* () {
			...
			while (true) {
				if (last) {
					let upMove = current.high - last.high;
					plusDm = format(upMove > last.low - current.low && upMove > 0 ? upMove : 0);
				}
				last = current;
...
//#endregion
//#region node_modules/technicalindicators/lib/directionalmovement/TrueRange.js
	...
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				previousClose = current.close;
				if (result != void 0) result = format(result);
				current = yield result;
			}
		})();
		this.generator.next();
...
//#endregion
//#region node_modules/technicalindicators/lib/directionalmovement/ADX.js
var ADX = class extends Indicator {
	constructor(input) {
		...
		if (!(lows.length === highs.length && highs.length === closes.length)) throw "Inputs(low,high, close) not of equal size";
		this.result = [];
		this.generator = (function* () {
			var tick = yield, lastPDI, lastMDI, lastDX, smoothedDX;
			while (true) {
				...
				if (lastATR != void 0 && lastAPDM != void 0 && lastAMDM != void 0) {
		...
		lows.forEach((tick, index) => {
			...
			if (result.value != void 0 && result.value.adx != void 0) this.result.push({
	...
	nextValue(price) {
		...
		if (result != void 0 && result.adx != void 0) return {
...
//#endregion
//#region node_modules/technicalindicators/lib/directionalmovement/ATR.js
	...
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				if (trange === void 0) avgTrueRange = void 0;
				else avgTrueRange = wema.nextValue(trange);
		...
		lows.forEach((tick, index) => {
			...
			if (result.value !== void 0) this.result.push(format(result.value));
		});
	}
	nextValue(price) {
		return this.generator.next(price).value;
...
//#endregion
//#region node_modules/technicalindicators/lib/momentum/ROC.js
	...
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				else roc = (tick - pastPeriods.lastShift) / pastPeriods.lastShift * 100;
				tick = yield roc;
			}
		})();
		this.generator.next();
		priceArray.forEach((tick) => {
			var result = this.generator.next(tick);
			if (result.value != void 0 && !isNaN(result.value)) this.result.push(this.format(result.value));
		});
	}
	nextValue(price) {
		var nextResult = this.generator.next(price);
		if (nextResult.value != void 0 && !isNaN(nextResult.value)) return this.format(nextResult.value);
	}
};
ROC.calculate = roc;
function roc(input) {
...
//#endregion
//#region node_modules/technicalindicators/lib/momentum/KST.js
var KST = class extends Indicator {
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				if (index < firstResult) index++;
				else kst = RCMA1 * 1 + RCMA2 * 2 + RCMA3 * 3 + RCMA4 * 4;
				signal = kst !== void 0 ? signalSMA.nextValue(kst) : void 0;
				result = kst !== void 0 ? {
					kst: format(kst),
					signal: signal ? format(signal) : void 0
				} : void 0;
	...
	nextValue(price) {
		...
		if (nextResult.value != void 0) return nextResult.value;
	}
};
KST.calculate = kst;
function kst(input) {
...
//#endregion
//#region node_modules/technicalindicators/lib/momentum/PSAR.js
...
//#endregion
//#region node_modules/technicalindicators/lib/momentum/Stochastic.js
...
//#endregion
//#region node_modules/technicalindicators/lib/momentum/WilliamsR.js
...
//#endregion
//#region node_modules/technicalindicators/lib/volume/ADL.js
/**
...
var ADL = class extends Indicator {
	constructor(input) {
		...
		if (!(lows.length === highs.length && highs.length === closes.length && highs.length === volumes.length)) throw "Inputs(low,high, close, volumes) not of equal size";
		this.result = [];
		this.generator = (function* () {
			var result = 0;
			var tick = yield;
...
//#endregion
//#region node_modules/technicalindicators/lib/volume/OBV.js
var OBV = class extends Indicator {
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				if (lastClose < tick.close) result = result + tick.volume;
				else if (tick.close < lastClose) result = result - tick.volume;
				lastClose = tick.close;
				tick = yield result;
			}
		})();
...
//#endregion
//#region node_modules/technicalindicators/lib/momentum/TRIX.js
/**
	...
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				tick = yield result ? format(result) : void 0;
			}
		})();
		this.generator.next();
		priceArray.forEach((tick) => {
...
//#endregion
//#region node_modules/technicalindicators/lib/volume/ForceIndex.js
var ForceIndex = class extends Indicator {
	constructor(input) {
		...
		if (!(volumes.length === closes.length)) throw "Inputs(volume, close) not of equal size";
...
//#endregion
//#region node_modules/technicalindicators/lib/oscillators/CCI.js
	...
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				let sum = 0;
				if (smaTp != void 0) {
					for (let x of currentTpSet.iterator()) sum = sum + Math.abs(x - smaTp);
...
//#endregion
//#region node_modules/technicalindicators/lib/oscillators/AwesomeOscillator.js
var AwesomeOscillator = class extends Indicator {
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				if (slowSmaValue !== void 0 && fastSmaValue !== void 0) result = fastSmaValue - slowSmaValue;
				tick = yield result;
			}
		})();
		this.generator.next();
	...
	nextValue(price) {
		...
		if (result.value != void 0) return this.format(result.value);
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/volume/VWAP.js
...
//#endregion
//#region node_modules/technicalindicators/lib/volume/VolumeProfile.js
		...
		for (let i = 0; i < bars; i++) {
			...
			for (let priceBar = 0; priceBar < highs.length; priceBar++) {
				...
				if (priceFallsBetweenBarRange(rangeStart, rangeEnd, priceBarStart, priceBarEnd)) {
					...
					if (priceBarOpen > priceBarClose) bearishVolume = bearishVolume + priceBarVolume;
					else bullishVolume = bullishVolume + priceBarVolume;
				}
			}
			this.result.push({
				rangeStart,
...
//#endregion
//#region node_modules/technicalindicators/lib/chart_types/TypicalPrice.js
/**
* Created by AAravindan on 5/4/16.
*/
var TypicalPrice = class extends Indicator {
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) priceInput = yield (priceInput.high + priceInput.low + priceInput.close) / 3;
		})();
		this.generator.next();
...
//#endregion
//#region node_modules/technicalindicators/lib/volume/MFI.js
/**
* Created by AAravindan on 5/17/16.
*/
var MFI = class extends Indicator {
	constructor(input) {
		...
		highs.forEach((tickHigh, index) => {
			...
			var result = this.generator.next(tickInput);
			if (result.value != void 0) this.result.push(parseFloat(result.value.toFixed(2)));
		});
	}
	nextValue(price) {
		var result = this.generator.next(price);
		if (result.value != void 0) return parseFloat(result.value.toFixed(2));
	}
};
MFI.calculate = mfi;
function mfi(input) {
...
//#endregion
//#region node_modules/technicalindicators/lib/momentum/StochasticRSI.js
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/Highest.js
	...
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				if (periodList.totalPushed >= period) high = periodList.periodHigh;
				tick = yield high;
			}
		})();
		this.generator.next();
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/Lowest.js
var Lowest = class extends Indicator {
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				periodList.push(tick);
				if (periodList.totalPushed >= period) high = periodList.periodLow;
				tick = yield high;
			}
		})();
		this.generator.next();
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/Sum.js
var Sum = class extends Indicator {
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				periodList.push(tick);
				if (periodList.totalPushed >= period) high = periodList.periodSum;
				tick = yield high;
			}
		})();
		this.generator.next();
...
//#endregion
//#region node_modules/technicalindicators/lib/chart_types/Renko.js
/**
* Created by AAravindan on 5/4/16.
*/
...
//#endregion
//#region node_modules/technicalindicators/lib/chart_types/HeikinAshi.js
/**
* Created by AAravindan on 5/4/16.
*/
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/CandlestickFinder.js
var CandlestickFinder = class {
	constructor() {}
	approximateEqual(a, b) {
		return parseFloat(Math.abs(a - b).toPrecision(4)) * 1 <= parseFloat((a * .001).toPrecision(4)) * 1;
	}
	logic(data) {
	...
	hasPattern(data) {
		...
		return this.logic.call(this, this._getLastDataForCandleStick(data));
	}
	_getLastDataForCandleStick(data) {
		let requiredCount = this.requiredCount;
		if (data.close.length === requiredCount) return data;
		else {
			let returnVal = {
				open: [],
				high: [],
	...
	_generateDataForCandleStick(data) {
		let requiredCount = this.requiredCount;
		return data.close.map(function(currentData, index) {
			let i = 0;
			let returnVal = {
				open: [],
				high: [],
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/MorningStar.js
var MorningStar = class extends CandlestickFinder {
	...
	logic(data) {
		...
		return firstdaysClose < firstdaysOpen && firstdaysLow > seconddaysLow && firstdaysLow > seconddaysHigh && seconddaysHigh < firstdaysLow && seconddaysLow < firstdaysLow && thirddaysOpen > seconddaysHigh && seconddaysClose < thirddaysOpen && thirddaysOpen < thirddaysClose && thirddaysClose > firstdaysMidpoint;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BullishEngulfingPattern.js
var BullishEngulfingPattern = class extends CandlestickFinder {
	...
	logic(data) {
		...
		data.high[1];
		data.low[1];
		return firstdaysClose < firstdaysOpen && firstdaysOpen > seconddaysOpen && firstdaysClose > seconddaysOpen && firstdaysOpen < seconddaysClose;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BullishHarami.js
var BullishHarami = class extends CandlestickFinder {
	...
	logic(data) {
		...
		let seconddaysHigh = data.high[1];
		let seconddaysLow = data.low[1];
		return firstdaysOpen > seconddaysOpen && firstdaysClose < seconddaysOpen && firstdaysClose < seconddaysClose && firstdaysOpen > seconddaysLow && firstdaysHigh > seconddaysHigh;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BullishHaramiCross.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/Doji.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/MorningDojiStar.js
var MorningDojiStar = class extends CandlestickFinder {
	...
	logic(data) {
		...
		return isFirstBearish && dojiExists && thirddaysOpen < thirddaysClose && seconddaysHigh < firstdaysLow && seconddaysLow < firstdaysLow && thirddaysOpen > seconddaysHigh && seconddaysClose < thirddaysOpen && thirddaysClose > firstdaysMidpoint;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/DownsideTasukiGap.js
var DownsideTasukiGap = class extends CandlestickFinder {
	...
	logic(data) {
		...
		data.high[2];
		data.low[2];
		return firstdaysClose < firstdaysOpen && seconddaysClose < seconddaysOpen && thirddaysClose > thirddaysOpen && seconddaysHigh < firstdaysLow && seconddaysOpen > thirddaysOpen && seconddaysClose < thirddaysOpen && thirddaysClose > seconddaysOpen && thirddaysClose < firstdaysClose;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BullishMarubozu.js
var BullishMarubozu = class extends CandlestickFinder {
	...
	logic(data) {
		...
		let daysHigh = data.high[0];
		let daysLow = data.low[0];
		return this.approximateEqual(daysClose, daysHigh) && this.approximateEqual(daysLow, daysOpen) && daysOpen < daysClose && daysOpen < daysHigh;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/PiercingLine.js
var PiercingLine = class extends CandlestickFinder {
	...
	logic(data) {
		...
		let seconddaysLow = data.low[1];
		let firstdaysMidpoint = (firstdaysOpen + firstdaysClose) / 2;
		return seconddaysLow < firstdaysLow && firstdaysClose < firstdaysOpen && firstdaysLow > seconddaysOpen && seconddaysClose > firstdaysMidpoint && seconddaysClose > seconddaysOpen;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/ThreeWhiteSoldiers.js
var ThreeWhiteSoldiers = class extends CandlestickFinder {
	...
	logic(data) {
		...
		data.low[2];
		return seconddaysHigh > firstdaysHigh && thirddaysHigh > seconddaysHigh && firstdaysOpen < firstdaysClose && seconddaysOpen < seconddaysClose && thirddaysOpen < thirddaysClose && firstdaysClose > seconddaysOpen && seconddaysOpen < firstdaysHigh && seconddaysHigh > thirddaysOpen && thirddaysOpen < seconddaysClose;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BullishHammerStick.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BullishInvertedHammerStick.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BearishHammerStick.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BearishInvertedHammerStick.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/HammerPattern.js
var HammerPattern = class extends CandlestickFinder {
	...
	hasConfirmation(data) {
		...
		return possibleConfirmation.open < possibleConfirmation.close && possibleHammer.close < possibleConfirmation.close;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/HammerPatternUnconfirmed.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/TweezerBottom.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/Bullish.js
var bullishPatterns = [
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BearishEngulfingPattern.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BearishHarami.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BearishHaramiCross.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/EveningDojiStar.js
var EveningDojiStar = class extends CandlestickFinder {
	...
	logic(data) {
		...
		return isFirstBullish && dojiExists && seconddaysHigh > firstdaysHigh && seconddaysLow > firstdaysHigh && thirddaysOpen < seconddaysLow && seconddaysClose > thirddaysOpen && thirddaysOpen > thirddaysClose && thirddaysClose < firstdaysMidpoint;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/EveningStar.js
var EveningStar = class extends CandlestickFinder {
	...
	logic(data) {
		...
		data.low[2];
		let firstdaysMidpoint = (firstdaysOpen + firstdaysClose) / 2;
		return firstdaysClose > firstdaysOpen && firstdaysHigh < seconddaysLow && firstdaysHigh < seconddaysHigh && seconddaysHigh > firstdaysHigh && seconddaysLow > firstdaysHigh && thirddaysOpen < seconddaysLow && seconddaysClose > thirddaysOpen && thirddaysOpen > thirddaysClose && thirddaysClose < firstdaysMidpoint;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BearishMarubozu.js
var BearishMarubozu = class extends CandlestickFinder {
	...
	logic(data) {
		...
		let daysHigh = data.high[0];
		let daysLow = data.low[0];
		return this.approximateEqual(daysOpen, daysHigh) && this.approximateEqual(daysLow, daysClose) && daysOpen > daysClose && daysOpen > daysLow;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/ThreeBlackCrows.js
var ThreeBlackCrows = class extends CandlestickFinder {
	...
	logic(data) {
		...
		return firstdaysLow > seconddaysLow && seconddaysLow > thirddaysLow && firstdaysOpen > firstdaysClose && seconddaysOpen > seconddaysClose && thirddaysOpen > thirddaysClose && firstdaysOpen > seconddaysOpen && seconddaysOpen > firstdaysClose && seconddaysOpen > thirddaysOpen && thirddaysOpen > seconddaysClose;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/HangingMan.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/HangingManUnconfirmed.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/ShootingStar.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/ShootingStarUnconfirmed.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/TweezerTop.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/Bearish.js
var bearishPatterns = [
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/AbandonedBaby.js
var AbandonedBaby = class extends CandlestickFinder {
	...
	logic(data) {
		...
		return isFirstBearish && dojiExists && seconddaysHigh < firstdaysLow && thirddaysLow > seconddaysHigh && thirddaysClose > thirddaysOpen && thirddaysHigh < firstdaysOpen;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/DarkCloudCover.js
var DarkCloudCover = class extends CandlestickFinder {
	...
	logic(data) {
		...
		return firstdaysClose > firstdaysOpen && seconddaysClose < seconddaysOpen && seconddaysOpen > firstdaysHigh && seconddaysClose < firstdayMidpoint && seconddaysClose > firstdaysOpen;
	}
};
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/DragonFlyDoji.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/GraveStoneDoji.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BullishSpinningTop.js
...
//#endregion
//#region node_modules/technicalindicators/lib/candlestick/BearishSpinningTop.js
...
//#endregion
//#region node_modules/technicalindicators/lib/drawingtools/fibonacci.js
/**
...
function fibonacciretracement(start, end) {
	...
	let retracements;
	if (start < end) retracements = levels.map(function(level) {
		let calculated = end - Math.abs(start - end) * level / 100;
		return calculated > 0 ? calculated : 0;
	});
	else retracements = levels.map(function(level) {
		let calculated = end + Math.abs(start - end) * level / 100;
		return calculated > 0 ? calculated : 0;
	});
	return retracements;
...
//#endregion
//#region node_modules/technicalindicators/lib/ichimoku/IchimokuCloud.js
	...
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				if (periodCounter < period) periodCounter++;
				else {
					...
					result = {
						...
						spanB: (currenSpanData.periodHigh + currenSpanData.periodLow) / 2
					};
				}
				tick = yield result;
			}
		...
		input.low.forEach((tick, index) => {
			...
			if (result.value) this.result.push(result.value);
		});
	}
	nextValue(price) {
		return this.generator.next(price).value;
...
//#endregion
//#region node_modules/technicalindicators/lib/volatility/KeltnerChannels.js
...
var KeltnerChannels = class extends Indicator {
	constructor(input) {
		super(input);
		var maProducer = new (input.useSMA ? SMA : EMA)({
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				if (ma != void 0 && atr != void 0) result = {
					middle: ma,
		...
		this.generator.next();
		input.high.forEach((tickHigh, index) => {
			var tickInput = {
				high: tickHigh,
				low: input.low[index],
				close: input.close[index]
...
//#endregion
//#region node_modules/technicalindicators/lib/volatility/ChandelierExit.js
	...
	constructor(input) {
		...
		this.generator = (function* () {
			...
			while (true) {
				...
				if (dataCollector.totalPushed >= 2 * input.period && atr != void 0) result = {
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/CrossUp.js
var CrossUp = class extends Indicator {
	constructor(input) {
		...
		const genFn = (function* () {
			...
			while (true) {
				...
				var pointer = 1;
				while (result === true && currentLineA[pointer] >= currentLineB[pointer]) if (currentLineA[pointer] > currentLineB[pointer]) result = false;
				else if (currentLineA[pointer] < currentLineB[pointer]) result = true;
				else if (currentLineA[pointer] === currentLineB[pointer]) pointer += 1;
				if (result === true) {
	...
	static reverseInputs(input) {
		if (input.reversedInput) {
			input.lineA && input.lineA.reverse();
			input.lineB && input.lineB.reverse();
		}
	}
...
//#endregion
//#region node_modules/technicalindicators/lib/Utils/CrossDown.js
